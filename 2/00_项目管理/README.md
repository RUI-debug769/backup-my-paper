...
+ 湿实验验证（荧光/CD/FTIR/GC-MS）
计算 + 实验闭环 → 独立投稿               实验+计算 → 需配合
```
...
│  "为什么会变化？"  — 因果层                                │
│  蛋白结构 → 对接 → MD → MM/PBSA                           │
│      └→ 荧光/CD/FTIR/GC-MS 湿实验交叉验证                  │
│               │                                         │
│               │ 结合自由能 + 残基贡献 + 温度效应             │
│               ↓                                         │
│  Paper 3 (跨物种推广)                                     │
│  "方法能否泛化？" — 泛化层                                 │
│  P1+P2 方法论 × 常见食用菌（香菇/平菇/杏鲍菇等）            │
│                                                         │
└─────────────────────────────────────────────────────────┘
```
...
Layer 6: 湿实验验证 🆕
  ├── 荧光光谱 → 验证 MD 预测的结合常数 (Ka)
  ├── 圆二色谱 CD → 验证蛋白质构象温度依赖性变化
  ├── FTIR → 验证蛋白质二级结构 (α-helix/β-sheet 比例)
  └── GC-MS 顶空定量 → 验证风味分子实际保留/释放量

Layer 7: 论文输出
  ├── 4-5 Figures + 2-3 Tables
...
| **湿实验验证** | 荧光光谱仪、圆二色谱 CD、FTIR、GC-MS（浙工商平台） |
...
2026年10月   ████████░░  湿实验验证 (荧光/CD/FTIR/GC-MS)
2026年11月   █████████░  Figures + Tables
2026年12月   手稿撰写 + 内审
