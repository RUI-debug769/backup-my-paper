...
```
Paper 1（大创）             Paper 2（MD计算）           Paper 3（整合）
2026-05 ──── 2027-05       2026-07 ──── 2027-01      2027 ──────── 2028
"什么条件风味好？"          "蛋白如何抓住风味？"        "多尺度如何统一？"
Condition → Quality        Atomic → Molecular         Multi-Scale Integration
Food Chemistry / CEA       Food Hydrocolloids         Nature Food / Nat Commun
实验 + AI 预测              纯计算 MD + MM/PBSA         四层框架整合
```
...
Layer 2 — 因果（Paper 2）: 蛋白构象 → 风味结合，MD 揭示
Layer 3 — 系统（Paper 3）: 原子→分子→宏观，多尺度整合
```
...
│
├── 2/                          ← Paper 2：MD 计算
│   ├── 00_项目管理/             README + Paper2_MVE报告 + Paper2_启动文档
│   ├── 01_搜/                   MD文献检索记录 + 关键词演化
│   ├── 04_验/                   ★ 蛋白结构 + 配体库 + 对接结果 + MD脚本
│   ├── 02_聚/03_分/05_合/...   (待填充)
│   └── raw/                     MD方法学参考PDF ×4
│
├── 3/                          ← Paper 3：Nature Food 多尺度整合
│   ├── 00_项目管理/             README + 目标期刊调研
│   ├── 01_搜/                   文献检索记录
│   └── (05_合 等骨架已建，待 1+2 结果)
│
...
```
2/
├── 00_项目管理/    README.md（项目总览）+ Paper2_MVE报告.md + Paper2_启动文档.md
├── 01_搜/          检索式记录.md + 关键词演化.md ✅ 已创建
├── 02_聚/          （待文献检索完成后填充）
├── 03_分/          （待文献分组完成后填充）
├── 04_验/          ★ MD工作核心：蛋白结构 + 配体 + 对接 + MD数据
│   ├── structures/              3 个 PDB
│   ├── ligands/                 SMILES + 物化性质
│   ├── docking_results/         Vina 对接结果 (13对)
│   ├── paper2_docking_pipeline.py  完整对接管线
│   ├── run_md_openmm.py         OpenMM MD 试跑
│   ├── run_production.sh        生产 MD 脚本
...
```
Paper 1 (实验+AI)  ──→  风味化合物清单  ──→  Paper 2 (MD) 的配体输入
Paper 2 (MD)       ──→  结合自由能 + 残基  ──→  Paper 3 (整合) 的 Layer 1
```

**解耦点**: Paper 2 不依赖 Paper 1 的任何实验数据来支撑其核心论点。MD 模拟自身形成完整证据链（对接→MD→MM/PBSA→氢键→残基分解）。

---

## 四、Paper 3 — Nature Food 后续项目当前状态
...
| **状态** | 🔬 Layer 1 (MD) 由 Paper 2 承担 — 本论文等待 Paper 1 + 2 结果 |
...
```
3/04_验/
（MD 计算工作已迁移至 2/04_验/，本目录现为整合论文规划空间）
```
...
Paper 1 必须产出：关键风味化合物清单、最优干燥条件、OAV 数据 — 这些是 Paper 3 的输入。
...
| **Paper 2 MVE 报告** | `2/00_项目管理/Paper2_MVE报告.md` |
| **Paper 2 项目总览** | `2/00_项目管理/README.md` |
| **Paper 2 检索式** | `2/01_搜/检索式记录.md` |
| Paper 3 大纲 | `3/Nature_Food_Proposal_Outline.docx` |
| Paper 3 总览 | `3/00_项目管理/README.md` |
| 目标期刊（P1） | `1/00_项目管理/目标期刊-调研.md` |
| 目标期刊（P3） | `3/00_项目管理/目标期刊-调研.md` |
