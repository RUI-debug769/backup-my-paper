...
```
Paper 1（大创）             Paper B（MD独立）          Paper 2（整合）
...
Layer 2 — 因果（Paper B）: 蛋白构象 → 风味结合，MD 揭示
Layer 3 — 系统（Paper 2）: 原子→分子→宏观，多尺度整合
```
...
| 蛋白结构 | ✅ 继承 Paper 2 | 3 PDB: MBL_lectin (92.2) / Tyrosinase (87.5) / H_lectin (89.5) |
| 配体库 | ✅ 继承 Paper 2 | 10 化合物 SMILES + 物化性质 |
...
```
3/
├── 00_项目管理/    README.md（项目总览）
├── 01_搜/          检索式记录.md + 关键词演化.md ✅ 已创建
├── 02_聚/          （待文献检索完成后填充）
├── 03_分/          （待文献分组完成后填充）
├── 04_验/          蛋白结构 + 配体 + 对接 + MD 数据
├── 05_合/          图表 + 手稿
...
```
Paper 1 (实验+AI)  ──→  风味化合物清单  ──→  Paper B (MD) 的配体输入
Paper B (MD)       ──→  结合自由能 + 残基  ──→  Paper 2 (整合) 的 Layer 1
```

**解耦点**: Paper B 不依赖 Paper 1 的任何实验数据来支撑其核心论点。MD 模拟自身形成完整证据链（对接→MD→MM/PBSA→氢键→残基分解）。

---

## 四、Paper 2 — Nature Food 后续项目当前状态
...
| **Paper B 项目总览** | `3/00_项目管理/README.md` |
| **Paper B 检索式** | `3/01_搜/检索式记录.md` |
